...
SimpleModel::SimpleModel(size_t size,int protect, int fd, size_t offset):
    ...
    int32_t status = ANeuralNetworksMemory_createFromFd(size+offset,protect,fd,0,&memoryModel_);

    if(status!=ANEURALNETWORKS_NO_ERROR){
        __android_log_print(ANDROID_LOG_ERROR,LOG_TAG,"AneuralNetworksMemory_createFromFd failed");
        return ;
    }
