...
public class UpdateActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        thngid=intent.getStringExtra("thngid");
        thng=new Thng();
    ...
    protected void onResume() {
        super.onResume();
        Log.d(MainActivity.TAG, "run: "+thngid);
        new Thread(new Runnable() {
            @Override
            public void run() {
                Log.d(MainActivity.TAG, "run: "+thngid);
                 thng=manager.thngService().thngReader(thngid).execute();
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if(thng!=null){
                            ...
                            mEditextColor.setText(thng.getCustomFields().get("color").toString());
                            mEditextDesc.setText(thng.getDescription());
                            mEditextModel.setText(thng.getCustomFields().get("license_plate").toString());
                        }else{
                            mEditextColor.setError("No data found");
                        }
                    }
                });
            }
        ...
        mBtnAdd.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        final Thng thng1=manager.thngService().thngUpdater(thngid,thng).execute();
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                if(thng1!=null) {
